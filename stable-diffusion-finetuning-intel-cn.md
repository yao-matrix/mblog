...
经过 5 分钟的训练，训得的模型就保存在本地了，我们可以直接用 `diffusers` 的 `pipeline` 加载该模型并进行图像生成。但这里，我们要使用 [Optimum Intel 和 OpenVINO](https://huggingface.co/docs/optimum/intel/inference) 以进一步对模型进行推理优化。正如[上一篇文章](https://huggingface.co/blog/intel-sapphire-rapids-inference)中所讨论的，优化后，仅用单颗 CPU 就能让你在不到 5 秒的时间内生成一幅图像！
